...
public class Product : Entity
{
    ...
    public Product(string sku, decimal price, int stock, Guid categoryId)
    {
    ...
    public void RenameSku(string newSku)
    {
    ...
    public void Recategorize(Guid newCategoryId)
    {
    ...
    public void UpdateImage(Guid imageId, string imageUrl, int sortOrder)
    {
    ...
    public void DeactivateImage(Guid imageId)
    {
        var image = _images.FirstOrDefault(x => x.Id == imageId);
        image?.Deactivate();
    }
    ...
    {
        if (collectionId == Guid.Empty) throw new DomainException("CollectionId is required");
        if (_collections.Any(x => x.CollectionId == collectionId)) return;
        _collections.Add(new ProductCollection(collectionId));
    }

    public void AddColor(Guid colorId)
    {
        if (colorId == Guid.Empty) throw new DomainException("ColorId is required");
    ...
    {
        if (tagId == Guid.Empty) throw new DomainException("TagId is required");
        if (_tags.Any(x => x.TagId == tagId)) return;
        _tags.Add(new ProductTag(tagId));
    }

    public void AddStyle(Guid styleId)
    {
        if (styleId == Guid.Empty) throw new DomainException("StyleId is required");
        if (_styles.Any(x => x.StyleId == styleId)) return;
        _styles.Add(new ProductStyle(styleId));
    }

    public void AddOccasion(Guid occasionId)
    {
        if (occasionId == Guid.Empty) throw new DomainException("OccasionId is required");
    ...
    public void ClearCollections() => _collections.Clear();
    public void ClearStyles() => _styles.Clear();
    public void ClearOccasions() => _occasions.Clear();
    ...
    public void RemoveSalePrice() => SalePrice = null;
